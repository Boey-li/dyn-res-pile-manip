...
public:
	...
	void Initialize(py::array_t<float> scene_params, int thread_idx = 0)
	{
        ...
        // void CreateSpringGrid(Vec3 lower, int dx, int dy, int dz, float radius, int phase, 
        // float stretchStiffness, float bendStiffness, float shearStiffness, Vec3 velocity, float invMass)
        if (fabric_type == 0) {
            CreateSpringGrid(
